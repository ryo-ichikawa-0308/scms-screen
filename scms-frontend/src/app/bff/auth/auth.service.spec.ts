...
import { LoginRequest, LoginResponse, AccessToken } from 'src/app/models/api.model';
...
let service: AuthService;
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      providers: [
        ...
        AuthService,
        { provide: CookieService, useValue: { get: jasmine.createSpy('get'), set: jasmine.createSpy('set'), delete: jasmine.createSpy('delete') } },
      ],
    });
  ...
  describe('login', () => {
    ...
    // ----------------------------------------------------------------------------------------------------
    // login() > 正常系: ログインに成功し、トークンが保存される
    // ----------------------------------------------------------------------------------------------------
    it('正常系: API成功時、アクセストークンとID情報が保存され、isLoggedInシグナルがtrueになる', (done) => {
    ...
    // ----------------------------------------------------------------------------------------------------
    // login() > 異常系: ログインAPIが失敗した場合
    // ----------------------------------------------------------------------------------------------------
    it('異常系: API失敗時、エラーがthrowされ、isLoggedInシグナルはfalseのまま', (done) => {
      service.login(loginRequest).subscribe({
  ...
  describe('refreshToken', () => {
    ...
    const MOCK_NEW_TOKEN: AccessToken = { accessToken: 'new_token_123', expiresIn: 3600 };

    beforeEach(() => {
      (cookieService.get as jasmine.Spy).and.returnValue(MOCK_REFRESH_TOKEN);
    });

    // ----------------------------------------------------------------------------------------------------
    // refreshToken() > 正常系: リフレッシュトークンで新しいアクセストークンを取得
    // ----------------------------------------------------------------------------------------------------
    it('正常系: リフレッシュトークンが有効な場合、新しいアクセストークンを取得・保存する', (done) => {
    ...
    // ----------------------------------------------------------------------------------------------------
    // refreshToken() > 異常系: リフレッシュAPIが失敗した場合
    // ----------------------------------------------------------------------------------------------------
    it('異常系: リフレッシュAPIが失敗した場合、ログアウト処理が呼ばれる', (done) => {
  ...
  describe('logout', () => {
    // ----------------------------------------------------------------------------------------------------
    // logout() > 正常系: すべてのトークンが削除され、isLoggedInシグナルがfalseになる
    // ----------------------------------------------------------------------------------------------------
    it('正常系: ログアウト時、セッションストレージとクッキーのトークンが削除される', () => {
      // 準備: ログイン状態をtrueにする（シグナルのテストのため）
